...
file_names <- paste0("west_", 1:12, ".txt")
...
west_codon_usage <- codon_means_normalized / sum(codon_means_normalized)
west_df <- data.frame(
  Codon = names(west_codon_usage),
  Frequency = as.numeric(west_codon_usage),
  Species = "West Nile Virus"
)


# Step 2: Combine both dataframes
combined_df <- rbind(human_df, west_df)
...
df <- data.frame(
  ...
  Type = rep(c("West Nile", "Human"), each = length(codon_frequencies))
)
...
west_sds <- codon_sds_normalized[common_codons]

# Calculate the difference (virus - human)
diff_df <- data.frame(
  Codon = common_codons,
  West = as.numeric(west_codon_usage[common_codons]),
  Human = as.numeric(human_codon_usage[common_codons]),
  West_SD = as.numeric(west_sds)
)
diff_df$Difference <- diff_df$West - diff_df$Human
diff_df$Lower <- diff_df$Difference - diff_df$West_SD
diff_df$Upper <- diff_df$Difference + diff_df$West_SD
...
ggplot(diff_df, aes(x = reorder(Codon, -Difference), y = Difference, fill = Difference > 0)) +
  ...
  scale_fill_manual(values = c("TRUE" = "#8dc0cd", "FALSE" = "#fc8d62"),
                    ...
                    name = "West Nile Usage") +
  theme_minimal() +
  labs(title = "Codon Usage Difference: West Nile vs. Human",
